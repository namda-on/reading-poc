...
- `scripts/build_vocab_expression.py` — **어휘→표현** 프로토타입 데이터 생성(레포 밖 CSV → `public/vocab-expression.data.json`). 소스는 `VE_EXPR_CSV`(기본 `유기적 통합모드 문장 (공유용) - 문장 제출(0827).csv`)·`VE_VOCAB_CSV`(`All_*.csv`)·`VE_GSE_CSV`(`gse_corrected_final_0624 - 보정결과_전체.csv`) 환경변수. 표현 CSV는 **그룹(id)당** 트리거 어휘(`trigger`,`trigger_dictseq`) + 표현 패턴(`표제어 또는 회화패턴`) + 문장 3~6개(`문장1`=트리거가 들어간 대표 문장) + 결과 카드(`카드 초록(형태)`/`회색1행(뜻)`/`회색2행(설명)`) 구조이며, 그룹 첫 행에만 값이 있어 **forward-fill**로 읽는다. `trigger_dictseq`로 어휘 CSV(`seq`)와 gse(`dictSeq`)에 조인해 어휘 예문·뜻·품사·CEFR·레벨을 붙인다. 정렬은 `trigger_newRank3`(빈도) 오름차순, 문항 수는 `VE_MAX_ITEMS`(기본 60, 0이면 전체 — 조건 통과 143그룹).
  ...
  **[3] 슬롯 치환 단계(`frame`·`baseSegs`·`apply[]`)도 함께 굽는다.** 목적은 슬롯을 맞히는 게 아니라 **유저가 프레임을 직접 산출**하는 것이므로 프레임의 문자 경계를 알아야 한다. `frame_literals`가 `form`을 슬롯(`______`/`~`) 기준으로 쪼개 리터럴 조각을 뽑고, `segment`가 각 문장을 `[{t:'frame'|'slot', s}]`로 분해한다(리터럴을 좌→우로 탐색, 나머지가 슬롯). **`form` 표기가 문장에 그대로 등장하지 않는 그룹은 제외한다** — `주어 + didn't + ______`처럼 한글 자리표시자가 섞이거나 `Are / Is + ...`처럼 프레임에 대안이 박힌 경우(`프레임불가` 269), 리터럴이 예문에 실재하지 않는 경우(`세그먼트실패` 196), 프레임이 `VE_MIN_FRAME_WORDS`(기본 2)단어 미만인 경우(`프레임짧음` 8). 661그룹 → **143그룹**이 세 조건(버전A·버전B·[3])을 모두 통과한다.
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 → [2] 표현 문장 배열 → [3]-1 처음 보는 문장에 프레임 넣기(탭) → [3]-2 또 다른 문장에 프레임 타이핑**(진행바 분모 = 문항수×4). 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **버전 A/B 토글**(`ve:ver`) + **진입점 하나** — 들어가면 문항마다 트리거 어휘와 표현이 계속 달라진다.
