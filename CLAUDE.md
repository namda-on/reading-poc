...
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 채우기 → [2] 표현 문장 배열**. 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **버전 A/B 토글**(`ve:ver`) + **진입점 하나** — 들어가면 문항마다 트리거 어휘와 표현이 계속 달라진다.
  **두 버전을 바꿔가며 비교한다** — 어휘 학습 문장을 무엇으로 쓰는지만 다르고 배열 단계·보상은 동일하다:
  ...
  - 진행 위치(`ve:pos`)는 **두 버전이 공유**한다 — 버전만 바꿔 같은 문항을 바로 비교할 수 있게. 홈 배치는 `탭 → 시작 카드(흐름·남은 문항 + 큰 CTA `.start-btn`) → 실험 옵션`. 진입 버튼이 다른 카드들과 같은 톤이면 눌러야 들어간다는 게 안 읽히므로 **CTA만 채운 색 버튼**으로 두고 옵션은 `실험 옵션` 헤더 아래 보조 영역에 둔다. `이어서 풀기`/`처음부터 다시 풀기`를 제공하고, 레벨은 문항 카드 좌상단 태그(`레벨 N`, gse 어휘 레벨)로만 표시한다. [1] 어휘는 **채팅 말풍선 카드**(좌상단 `레벨 N` 태그·우상단 `New` 배지·상단 파란→흰 그라데이션): 한국어 번역(정답 뜻 영역 초록 `.tgt`, 괄호 설명줄 muted) 위, 영어 예문의 `[단어]`를 **인라인 입력 빈칸**(`<input class=blank>`, `field-sizing:content`로 자동 폭)으로 보여준다. 하단은 `? 힌트 보기` / `🎤 음성 모드`(비작동). 힌트는 **순차** — `? 힌트 보기`(**앞 2글자**+나머지 길이) → `✔ 정답 보기`(정답 전체, `.blank.revealed`). 둘 다 **placeholder로** 보여주므로 입력값이 남아 있으면 가려져 아무 반응이 없는 것처럼 보인다. 그래서 **정답 보기를 누를 때만 입력값과 오답 박스를 지워** 정답 placeholder가 보이게 하고(힌트 보기는 입력값을 유지), 그 뒤 다시 타이핑해도 정답이 가려지지 않게 **문장 아래에 `정답 {단어}` 주석 한 줄**(`#revealNote`)을 남긴다(정답을 맞히면 숨김). **정답 입력 시 확인 없이 즉시 처리**(`norm`), 오답은 **카드 안 파란 박스**에 `{입력} - 정답과 N글자 달라요`로 틀린 글자 빨강(`diffSpell`). 정답이면 **카드가 초록으로 바뀌고 ✓ 체크** → 정답 단어 초록 박스 + 하단이 **탭(다른 예문·오답 노트·AI 질문 답변·사전 검색·단어 정보) + 3버튼(단어 듣기·다시 듣기·다음 문제)**로 교체(다음 문제만 작동). 빈칸 정답은 학습 대상 어휘와 같아야 하므로 다른 단어를 괄호친 예문은 빌드 제외(버전 B는 문장에 쓰인 굴절형이 정답). 버전 B는 표현 문장 번역에 `[뜻]` 마크업이 없어 **`빈칸 뜻 {뜻}` 줄**(`.b-hint`)로 같은 만큼의 단서를 준다. [2] 배열은 **말풍선 카드**(한국어 문장 뜻) + **인라인 문장 채우기**(빈 밑줄에 뱅크 단어를 순서대로 탭해 채움, 종결부호 고정) + **단어 뱅크(자리 유지 — 채운 타일은 회색 빈칸으로 남김)** + `정답 보기`/`확인`. 타일은 소문자(첫 단어 노출 방지). 방해 단어는 현재 없음(데이터 준비되면 추가).
  **어휘→표현 연결은 텍스트 설명이 아니라 행위로 체감시킨다**(긴 안내 문구는 한 번에 안 읽히고, 트리거 어휘가 표현의 핵심이 아닐 때 어색해짐):
  ...
  - 배열 전 보라 박스(`learnBoxHtml`)는 **뭘 배우는지만** 짧게 보여준다 — `이번에 배울 표현` 라벨 + **패턴 한국어 뜻**(`pattern.meaning`) 두 줄. 상황 태그(일상/여행/비즈니스)와 활용 규모 수치(`N가지`)는 **쓰지 않는다** — 태그는 콘텐츠에서 없어질 수 있고, 수치는 실제 용례가 더 많을 수 있어 의미가 없다. **영어 프레임(`pattern.form`)은 정답 어순이 새므로 배열 전엔 절대 노출하지 않는다.** 어휘 연결은 문구가 아니라 칩 강조가 담당.
  - **방금 배운 어휘 칩을 초록으로 강조**(`.tile.trigger` + 배지 + 펄스). 배지 내용은 **옵션**(`ve:chipLabel`: `learned`=`방금 배운` / `meaning`=그 어휘의 뜻, 기본 `meaning`)으로 고르며, 홈의 세그먼트 컨트롤과 세션 중 상단 기어(설정 팝오버) 양쪽에서 바꿀 수 있고 세션 중 전환 시 즉시 반영된다(`applyChipLabels`, CSS `content:attr(data-label)`). 뜻 라벨은 `shortMeaning`이 `<주석>`·`(부연)`을 떼고 앞쪽 뜻만 10자 내로 줄인다. 어휘 단계의 초록 정답과 색을 이어 "그 단어"임을 인지시킨다. 어느 타일이 그 어휘인지는 `isTriggerToken`이 판정 — 정확일치 + 규칙 굴절형(days/going/taxes/fatter/dancing/parties, 소유격 `toilet's`, 복합어 `Round-trip⊃trip`)까지 인정하고 **하이픈 조각 단위로 비교**해 `wondering⊃ring` 같은 오탐을 막는다. 현재 데이터에선 60/60 문항 모두 강조 가능하다.
  - 그 칩을 문장에 넣는 순간 **'써먹었다' 연출**(`fireUsedEffect`): 팝 + 링 반사 + `방금 배운 단어!` 배지 + 햅틱(`navigator.vibrate`, Android만). 문항당 1회만 발동(반복 피로 방지).
  - 정답 시 **초록 카드 + ✓ + 완성 문장**(트리거 어휘만 초록 — 이 데이터엔 핵심 표현 구간 마크업이 없어 문장 내 형광펜은 쓰지 않는다) 아래에 **보상 카드**(`patternPayoffHtml`)를 붙인다: `🎉 새로운 표현을 배웠어요` + **재사용 프레임 공개**(`pattern.form`의 `______`/`~`를 빈칸으로 그린 틀 + `pattern.meaning`) + `pattern.desc`가 있으면 문법 포인트 한 줄 + `이 표현으로 이렇게도 말할 수 있어요` + **응용 예문 2개**(`siblings`). 배열 중엔 숨겨둔 영어 틀을 정답 후에 처음 공개하는 구성이라 보상에 새 정보가 담긴다.
  - 데이터 스키마: `items[]` = `{trigger,word,meaning,pos,cefr,level,rank, vocabA{answer,enLines,koLines}, vocabB{answer,enLines,koLines,hint}, sentence{en,kr,trigger}, pattern{form,meaning,desc}, siblings[{en,kr}]}`. TTS·음성 등 앱 부가기능은 시각적으로만 재현(비작동), 리딩·타이핑 중심.
