...
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). 앱(말해보카)의 어휘/문법 학습 UX를 따른다. 흐름은 아이템마다 **[1] 어휘 빈칸 채우기 → [2] 문장 배열**. 홈은 **표현/문법 탭** × **레벨 카드**(완료는 `localStorage` `ve:cleared`로 ✓ 배지·`n/N 완료`, 마지막 탭은 `ve:mode`). [1] 어휘 카드는 채팅 말풍선에 **한국어 번역(정답 뜻 영역 초록 `.tgt` 하이라이트 + 괄호 설명줄 muted)** 위, 영어 예문의 `[단어]`를 빈칸으로 아래에 보여준다. 힌트는 **순차 노출** — 처음엔 `💡 힌트 보기`만, 누르면 첫 글자+길이가 드러나며 그때 `👁 정답 보기`가 등장(전체 노출해도 직접 입력해야 함). **정답을 입력하면 확인 없이 즉시 정답 처리**(`norm`: 대소문자·앞뒤 문장부호 무시), 오답은 `정답과 N글자 달라요`로 틀린 글자를 빨강 강조(`diffSpell`). 빈칸 정답은 학습 대상 표제어와 같아야 하므로 굴절형·부분구 예문(be→[am] 등)은 빌드에서 제외. [2]는 방금 배운 어휘가 트리거하는 `sentence`(`[...]{...}` 마크업 제거)를 단어 타일로 섞어 배열(타일 소문자 — 첫 단어 노출 방지, 종결부호 제거; **방해 단어는 현재 없음** — 데이터 준비되면 추가 예정). **트리거 안내는 단어가 아니라 패턴을 전면에** 둔다(`triggerBoxHtml`) — "방금 배운 {word}, 이런 표현에서 이렇게 써요" + **패턴 칩**(표현: `title_en`(`~`)·`title_kr` / 문법: 소제목 문법 포인트). 단어가 부수적 필러인 경우(예: summer)의 어색함을 없애고, 재사용 가능한 패턴을 배운다는 프레이밍. 정답 시 원문 대소문자로 복원+트리거 초록 강조하고, **일반화 줄**(`genLineHtml`)로 같은 패턴의 다른 단어(`siblings`)를 보여줘 학습 효용감을 강화한다. 데이터 스키마: `modes.{expression,grammar}.levels[].items[]` = `{word,meaning,pos,cefr,vocab{answer,enLines,koLines},sentence{en,kr,trigger,pattern_en?,pattern_kr,siblings,...}}`. TTS 없음(리딩·타이핑 중심).
