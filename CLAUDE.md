...
- `scripts/build_vocab_expression.py` — **어휘→표현** 프로토타입 데이터 생성(레포 밖 CSV → `public/vocab-expression.data.json`). 소스는 `VE_EXPR_CSV`(기본 `유기적 통합모드 문장 (공유용) - 문장 제출(0827).csv`)·`VE_VOCAB_CSV`(`All_*.csv`)·`VE_GSE_CSV`(`gse_corrected_final_0624 - 보정결과_전체.csv`) 환경변수. 표현 CSV는 **그룹(id)당** 트리거 어휘(`trigger`,`trigger_dictseq`) + 표현 패턴(`표제어 또는 회화패턴`) + 문장 3~6개(`문장1`=트리거가 들어간 대표 문장) + 결과 카드(`카드 초록(형태)`/`회색1행(뜻)`/`회색2행(설명)`) 구조이며, 그룹 첫 행에만 값이 있어 **forward-fill**로 읽는다. `trigger_dictseq`로 어휘 CSV(`seq`)와 gse(`dictSeq`)에 조인해 어휘 예문·뜻·품사·CEFR·레벨을 붙인다. 정렬은 `trigger_newRank3`(빈도) 오름차순, 문항 수는 `VE_MAX_ITEMS`(기본 60, 0이면 전체 — 조건 통과 457그룹).
  ...
  **[3] 슬롯 치환 단계(`frame`·`baseSegs`·`apply[]`)도 함께 굽는다.** 목적은 슬롯을 맞히는 게 아니라 **유저가 프레임을 직접 산출**하는 것이므로 프레임의 문자 경계를 알아야 한다. **프레임은 `form`(J열)이 아니라 학습 문장(D열)에서 직접 추출한다** — `form`에는 `주어 + didn't + ______`처럼 한글 자리표시자나 `What / Who / Where`처럼 대안이 박힌 표기가 많아 문장에 그대로 등장하지 않는다(J열 기준으로는 661→143만 통과했다). `frame_candidates`가 문장1과 다른 문장의 **공통 연속 구간**을 `difflib.SequenceMatcher`로 뽑고, `extract_frame`이 문장1을 반드시 포함하면서 가장 많은 문장을 커버하는 후보를 고른다(우선순위: 프레임이 길수록 → 커버가 많을수록). **단일 연속 구간 `VE_MIN_FRAME_WORDS`(기본 2)단어 이상만** 인정한다 — 1단어 조각 두 개를 허용하면 `Can/Could you ~?`에서 `you`+`me`가 뽑혀 정작 패턴을 잃는다. `segment`가 각 문장을 `[{t:'frame'|'slot', s}]`로 분해한다(리터럴을 좌→우로 탐색, 나머지가 슬롯). 추출된 프레임은 `form`보다 **구체적**이다(`주어 + didn't + ___` → `I didn't`, `Do / Does + 주어 + ___ ?` → `Do you`) — [3]에는 구체적인 청크가 유리하다. 661그룹 → **457그룹** 통과, 제외 204(공통프레임없음 155 · 버전A불가 43 · 버전B불가 4 · 조인실패 2).
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 → [2] 표현 문장 배열 → [3]-1 처음 보는 문장에 프레임 넣기(탭) → [3]-2 또 다른 문장에 프레임 타이핑**. **[3]-2는 프레임을 공유하는 문장이 2개 이상인 문항에서만** 돌므로(457문항 중 211) 문항마다 단계 수가 3 또는 4다 — `buildStepIndex`가 세션 시작 시 총 단계 수를 합산하고 `setProgress`는 **문항 내 단계 위치**만 받아 누적 위치로 환산한다(고정 분모를 쓰면 3단계 문항에서 진행바가 튄다). 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **버전 A/B 토글**(`ve:ver`) + **진입점 하나** — 들어가면 문항마다 트리거 어휘와 표현이 계속 달라진다.
