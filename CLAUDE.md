...
- `scripts/build_vocab_expression.py` — **어휘→표현** 프로토타입 데이터 생성(레포 밖 CSV/xlsx → `public/vocab-expression.data.json`). 소스 경로는 `VE_EXPR_CSV`/`VE_GRAM_CSV`/`VE_VOCAB_CSV`/`VE_GSE_XLSX` 환경변수(기본 `~/Downloads`). 조인: 표현/문법 CSV의 `content_word`(트리거 어휘)+`sentence`(배열 문장)를 **어휘 CSV(`All_*.csv`)**에 `dictSeq==seq`로 조인해 어휘 학습 데이터(`learnSentence` 영어 예문의 `[단어]`=빈칸 정답, `learnSentenceMeaning` 한국어 번역의 `[뜻]`=초록 하이라이트, `meaning`, `pos`)를 붙이고, gse xlsx에서 CEFR만 보강. **문항마다 어휘와 표현/문법이 모두 달라야 하므로 패턴(표현 제목 / 문법 큰제목::소제목)당 1문항만** 뽑아 레벨 오름차순 **단일 시퀀스**로 만든다(레벨별로 묶으면 같은 패턴이 연속돼 키워드만 바뀌는 반복이 됨). 어휘도 중복 제거. 모드별 문항 수는 `VE_MAX_ITEMS`(기본 60, 0이면 전체 — 확보 가능한 패턴은 표현 267·문법 294개). 어휘 CSV의 `filter`가 `sexual`/`unnecessary`인 행은 제외. 표현 제목(`title_en`/`title_kr`, `~` 슬롯 패턴)·문법 소제목(`sub`)은 병합셀이라 **forward-fill**하고(문법은 큰제목 바뀌면 소제목 리셋). 같은 패턴의 다른 예문을 `siblings`(2개, `{word,en,kr}`)로, 그 패턴의 **원본 CSV 예문 수**를 `patternCount`로 붙인다(활용 규모 표시용 — 어휘 조인 성공 여부와 무관하게 세므로 실제 콘텐츠 기준. 조인 통과분만 세면 파이프라인 필터 때문에 과소집계됨).
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘/문법 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 채우기 → [2] 문장 배열**. 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **표현/문법 탭**(마지막 탭 `ve:mode`) + **진입점 하나**(레벨 카드 없음) — 들어가면 문항마다 트리거 어휘와 표현/문법이 계속 달라진다. 진행 위치는 모드별로 `ve:pos`에 저장해 `이어서 풀기`/`처음부터 다시 풀기`를 제공하고, 레벨은 문항 카드 좌상단 태그(`레벨 N`)로만 표시한다. [1] 어휘는 **채팅 말풍선 카드**(좌상단 `레벨 N` 태그·우상단 `New` 배지·상단 파란→흰 그라데이션): 한국어 번역(정답 뜻 영역 초록 `.tgt`, 괄호 설명줄 muted) 위, 영어 예문의 `[단어]`를 **인라인 입력 빈칸**(`<input class=blank>`, `field-sizing:content`로 자동 폭)으로 보여준다. 하단은 `? 힌트 보기` / `🎤 음성 모드`(비작동). 힌트는 **순차** — `힌트 보기`(첫 글자+길이 placeholder) → 누르면 `정답 보기`(전체 노출)로 바뀜. **정답 입력 시 확인 없이 즉시 처리**(`norm`), 오답은 **카드 안 파란 박스**에 `{입력} - 정답과 N글자 달라요`로 틀린 글자 빨강(`diffSpell`). 정답이면 **카드가 초록으로 바뀌고 ✓ 체크** → 정답 단어 초록 박스 + 하단이 **탭(다른 예문·오답 노트·AI 질문 답변·사전 검색·단어 정보) + 3버튼(단어 듣기·다시 듣기·다음 문제)**로 교체(다음 문제만 작동). 빈칸 정답은 표제어와 같아야 하므로 굴절형·부분구 예문(be→[am] 등)은 빌드 제외. [2] 배열은 **말풍선 카드**(한국어 A 대사) + **인라인 문장 채우기**(빈 밑줄에 뱅크 단어를 순서대로 탭해 채움, 종결부호 고정) + **단어 뱅크(자리 유지 — 채운 타일은 회색 빈칸으로 남김)** + `정답 보기`/`확인`. 타일은 소문자(첫 단어 노출 방지). 방해 단어는 현재 없음(데이터 준비되면 추가).
  **어휘→표현 연결은 텍스트 설명이 아니라 행위로 체감시킨다**(긴 안내 문구는 한 번에 안 읽히고, 트리거 어휘가 표현의 핵심이 아닐 때 어색해짐):
  - 배열 전 보라 박스(`learnBoxHtml`)는 **뭘 배우는지 + 얼마나 쓸모있는지**를 보여준다 — `이번에 배울 표현/문법` 라벨 · **패턴 한국어 뜻**(`pattern_kr`) · **활용 규모**(`patternCount`≥3일 때 `단어만 바꿔 N가지` / 문법 `이 문법으로 N개 문장`). 패턴 뜻 한 줄만으로는 효용이 안 느껴져서 규모를 함께 준다. 상황 태그(일상/여행/비즈니스)는 **쓰지 않는다**(콘텐츠에서 없어질 수 있는 분류). **영어 프레임(`pattern_en`)은 정답 어순이 새므로 배열 전엔 절대 노출하지 않는다.** 어휘 연결은 문구가 아니라 칩 강조가 담당.
  - **방금 배운 어휘 칩을 초록으로 강조**(`.tile.trigger` + `방금 배운` 배지 + 펄스). 어휘 단계의 초록 정답과 색을 이어 "그 단어"임을 인지시킨다. 어느 타일이 그 어휘인지는 `isTriggerToken`이 판정 — 정확일치 + 규칙 굴절형(days/going/taxes/fatter/dancing/parties, 소유격 `toilet's`, 복합어 `Round-trip⊃trip`)까지 인정하고 **하이픈 조각 단위로 비교**해 `wondering⊃ring` 같은 오탐을 막는다. 현재 커버리지 표현 150/150 · 문법 146/150(`spoke`·`best`·`said`·`gone` 불규칙은 미매칭 → 강조·연출 생략하는 폴백).
  - 그 칩을 문장에 넣는 순간 **'써먹었다' 연출**(`fireUsedEffect`): 팝 + 링 반사 + `방금 배운 단어!` 배지 + 햅틱(`navigator.vibrate`, Android만). 문항당 1회만 발동(반복 피로 방지).
  - 정답 시 **초록 카드 + ✓ + 완성 문장(트리거 초록)** 아래에 **보상 카드**(`patternPayoffHtml`)를 붙인다 — `🎉 이제 이 표현/문법을 쓸 수 있어요` + **재사용 프레임 공개**(표현은 `pattern_en`의 `~`를 빈칸으로 그린 틀 + 한국어 뜻, 문법은 문법 포인트) + `단어만 바꾸면 N가지로 말할 수 있어요` + **응용 예문 2개**(`siblings`의 `en`/`kr`, 바뀐 단어만 초록 — `hlWord`가 굴절형 매칭 + 문장부호 제외). 배열 중엔 숨겨둔 영어 틀을 정답 후에 처음 공개하는 구성이라 보상에 새 정보가 담긴다(단순 재진술이면 효용이 안 느껴짐). 데이터 스키마: `modes.{expression,grammar}.levels[].items[]` = `{word,meaning,pos,cefr,vocab{answer,enLines,koLines},sentence{en,kr,trigger,pattern_en?,pattern_kr,siblings?,...}}`. TTS·음성 등 앱 부가기능은 시각적으로만 재현(비작동), 리딩·타이핑 중심.
