...
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 채우기 → [2] 표현 문장 배열 → [3] 처음 보는 문장에 그 프레임 직접 쓰기**(진행바 분모 = 문항수×3). 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **버전 A/B 토글**(`ve:ver`) + **진입점 하나** — 들어가면 문항마다 트리거 어휘와 표현이 계속 달라진다.
  ...
  **어휘→표현 연결은 텍스트 설명이 아니라 행위로 체감시킨다**(긴 안내 문구는 한 번에 안 읽히고, 트리거 어휘가 표현의 핵심이 아닐 때 어색해짐):
  ...
  - **[3] 슬롯 치환(`renderApply`)** — 패턴 학습의 핵심은 슬롯 치환 능력인데 [2]까지는 치환을 한 번도 시키지 않는다. [3]은 **처음 보는 문장**(`apply[0]`)에서 같은 프레임을 다시 쓰게 한다. **여기서 유저가 산출해야 하는 건 프레임이다** — 전이되는 자산이 프레임이고 슬롯 단어(`menu`)는 부수적이며 모르는 단어일 수도 있어서, **슬롯은 회색 칩(`.fill-given`)으로 주어진 채 고정하고 프레임 자리만 빈칸으로 비운다**. 그래서 [2]와 조작은 닮았지만 인지 과제가 반대다 — [2]는 프레임을 **숨기고** 어순을 회상시키고, [3]은 프레임을 **비우고** 새 뜻에 얹게 한다. 프레임은 단어 단위로 쪼개 뱅크에 올리므로 어순도 과제에 포함되고, 프레임 조각이 둘 이상이면(`put ___ on`) 빈칸 그룹이 슬롯 칩을 사이에 두고 떨어져 나타난다. 정답 후 원문 대소문자로 교정한다(뱅크 타일은 소문자). 방해 단어는 없다.
  - **[3] 결과창(`applyPayoffHtml`)이 효용 체감을 담당한다** — 뜻이 전혀 다른 두 문장([2]의 대표문장 + [3]의 응용문장)을 나란히 놓고 **유저가 두 번 다 직접 놓은 프레임만 초록**으로 묶는다(`.pf-cmp-en`은 본문을 어둡게 두고 `.tgt`만 초록 — `patternPayoffHtml`의 `.pf-ex-en`은 '전부 초록'이 의도라 클래스를 분리했다). 근거를 코퍼스 수치가 아니라 **자기 산출물**로 두는 것이 요점이다.
