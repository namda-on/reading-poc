...
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘/문법 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 채우기 → [2] 문장 배열**. 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **표현/문법 탭**(마지막 탭 `ve:mode`) + **진입점 하나**(레벨 카드 없음) — 들어가면 문항마다 트리거 어휘와 표현/문법이 계속 달라진다. 진행 위치는 모드별로 `ve:pos`에 저장해 `이어서 풀기`/`처음부터 다시 풀기`를 제공하고, 레벨은 문항 카드 좌상단 태그(`레벨 N`)로만 표시한다. [1] 어휘는 **채팅 말풍선 카드**(좌상단 `레벨 N` 태그·우상단 `New` 배지·상단 파란→흰 그라데이션): 한국어 번역(정답 뜻 영역 초록 `.tgt`, 괄호 설명줄 muted) 위, 영어 예문의 `[단어]`를 **인라인 입력 빈칸**(`<input class=blank>`, `field-sizing:content`로 자동 폭)으로 보여준다. 하단은 `? 힌트 보기` / `🎤 음성 모드`(비작동). 힌트는 **3단계 순차**(같은 버튼 자리에서 바뀜) — `? 힌트 보기`(**앞 2글자**+나머지 길이 placeholder) → `✔ 정답 보기`(정답 전체 노출, `.blank.revealed`) → `→ 넘어가기`. **정답을 몰라도 막히지 않게** 정답을 본 뒤엔 넘어갈 수 있고, 이때는 맞힘으로 치지 않아 배너 칩이 초록 `✓ {단어}` 대신 중립 `1 {단어}`(`.sb-step.skipped`)로 표시된다(`itemSkipped`로 배열 단계까지 상태 유지). **정답 입력 시 확인 없이 즉시 처리**(`norm`), 오답은 **카드 안 파란 박스**에 `{입력} - 정답과 N글자 달라요`로 틀린 글자 빨강(`diffSpell`). 정답이면 **카드가 초록으로 바뀌고 ✓ 체크** → 정답 단어 초록 박스 + 하단이 **탭(다른 예문·오답 노트·AI 질문 답변·사전 검색·단어 정보) + 3버튼(단어 듣기·다시 듣기·다음 문제)**로 교체(다음 문제만 작동). 빈칸 정답은 표제어와 같아야 하므로 굴절형·부분구 예문(be→[am] 등)은 빌드 제외. [2] 배열은 **말풍선 카드**(한국어 A 대사) + **인라인 문장 채우기**(빈 밑줄에 뱅크 단어를 순서대로 탭해 채움, 종결부호 고정) + **단어 뱅크(자리 유지 — 채운 타일은 회색 빈칸으로 남김)** + `정답 보기`/`확인`. 타일은 소문자(첫 단어 노출 방지). 방해 단어는 현재 없음(데이터 준비되면 추가).
  **어휘→표현 연결은 텍스트 설명이 아니라 행위로 체감시킨다**(긴 안내 문구는 한 번에 안 읽히고, 트리거 어휘가 표현의 핵심이 아닐 때 어색해짐):
  - 연결 강화 장치 3종은 **개별 on/off 옵션**(`ve:opts` = `{banner,preview,fly}`, 기본 전부 ON)으로 조합을 비교할 수 있다. 홈 옵션 박스와 세션 중 상단 기어 팝오버 양쪽에서 토글한다.
    ...
    - `preview` — 어휘 정답 직후 **다음 단계 예고**: 하단에 `💡 이제 이 단어로 실제 표현을 만들어봐요` + 세 번째 버튼 문구를 `다음 문제` → `{단어} 써보기`로.
  ...
  - **방금 배운 어휘 칩을 초록으로 강조**(`.tile.trigger` + 배지 + 펄스). 배지 내용은 **옵션**(`ve:chipLabel`: `learned`=`방금 배운` / `meaning`=그 어휘의 뜻)으로 고르며, 홈의 세그먼트 컨트롤과 세션 중 상단 기어(설정 팝오버) 양쪽에서 바꿀 수 있고 세션 중 전환 시 즉시 반영된다(`applyChipLabels`, CSS `content:attr(data-label)`). 뜻 라벨은 `shortMeaning`이 `<주석>`·`(부연)`을 떼고 앞쪽 뜻만 10자 내로 줄인다. 어휘 단계의 초록 정답과 색을 이어 "그 단어"임을 인지시킨다. 어느 타일이 그 어휘인지는 `isTriggerToken`이 판정 — 정확일치 + 규칙 굴절형(days/going/taxes/fatter/dancing/parties, 소유격 `toilet's`, 복합어 `Round-trip⊃trip`)까지 인정하고 **하이픈 조각 단위로 비교**해 `wondering⊃ring` 같은 오탐을 막는다. 매칭률은 표현 100% · 문법 대부분이고, `spoke`·`best`·`said`·`gone` 같은 불규칙만 미매칭 → 강조·연출을 생략하는 폴백으로 동작한다.
