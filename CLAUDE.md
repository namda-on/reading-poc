...
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). **앱(말해보카)의 어휘 학습 UI·UX를 화면 그대로 재현**(참고 영상 기반). 흐름은 아이템마다 **[1] 어휘 빈칸 → [2] 표현 문장 배열 → [3]-1 처음 보는 문장에 프레임 넣기(탭) → [3]-2 또 다른 문장에 프레임 타이핑**. **[3]-2는 프레임을 공유하는 문장이 2개 이상인 문항에서만** 돌므로(526문항 중 약 250) 문항마다 단계 수가 3 또는 4다 — `buildStepIndex`가 세션 시작 시 총 단계 수를 합산하고 `setProgress`는 **문항 내 단계 위치**만 받아 누적 위치로 환산한다(고정 분모를 쓰면 3단계 문항에서 진행바가 튄다). 상단바는 트로피·진행바(별 마커·`i/N`)·설정. 홈은 **버전 A/B 토글**(`ve:ver`) + **진입점 하나** — 들어가면 문항마다 트리거 어휘와 표현이 계속 달라진다.
  ...
  **어휘→표현 연결은 텍스트 설명이 아니라 행위로 체감시킨다**(긴 안내 문구는 한 번에 안 읽히고, 트리거 어휘가 표현의 핵심이 아닐 때 어색해짐):
  ...
  - **[3] 슬롯 치환(`renderApply(round)`)** — 패턴 학습의 핵심은 슬롯 치환 능력인데 [2]까지는 치환을 한 번도 시키지 않는다. [3]은 **처음 보는 문장**에서 같은 프레임을 다시 쓰게 하며, **한 번은 체감이 덜하므로 2회차까지 간다**: `round 0`은 `apply[0]`을 **뱅크 탭**(프레임을 단어로 쪼개 올려 어순도 과제), `round 1`은 `apply[1]`을 **뱅크 없이 타이핑**(`typing`, 프레임 조각 단위 `<input class=blank>`)으로 승급시킨다. 타이핑 라운드는 [1] 어휘와 같은 규약을 쓴다 — 정답 입력 시 확인 없이 즉시 처리, 오답은 `wrong-box`+`diffSpell`, `힌트 보기`(앞 2글자) → `정답 보기`(입력값 지우고 placeholder + `#revealNote`). 프레임 조각이 둘이면 입력칸도 둘이고 **둘 다 맞아야** 통과한다. `round 0` 정답 후 버튼은 `한 번 더`다. **여기서 유저가 산출해야 하는 건 프레임이다** — 전이되는 자산이 프레임이고 슬롯 단어(`menu`)는 부수적이며 모르는 단어일 수도 있어서, **슬롯은 회색 칩(`.fill-given`)으로 주어진 채 고정하고 프레임 자리만 빈칸으로 비운다**. 그래서 [2]와 조작은 닮았지만 인지 과제가 반대다 — [2]는 프레임을 **숨기고** 어순을 회상시키고, [3]은 프레임을 **비우고** 새 뜻에 얹게 한다. 프레임은 단어 단위로 쪼개 뱅크에 올리므로 어순도 과제에 포함되고, 프레임 조각이 둘 이상이면(`put ___ on`) 빈칸 그룹이 슬롯 칩을 사이에 두고 떨어져 나타난다. 정답 후 원문 대소문자로 교정한다(뱅크 타일은 소문자). 방해 단어는 없다.
  - **[3] 결과창(`applyPayoffHtml(item, rows)`)이 효용 체감을 담당한다** — 지금까지 만든 문장을 나란히 놓고 **유저가 매번 직접 놓은 프레임만 초록**으로 묶는다. **줄 수가 곧 반복 횟수**여서 `round 0`은 2문장(`같은 틀로 두 번…`), `round 1`은 3문장(`세 번…`)으로 늘어난다(`.pf-cmp-en`은 본문을 어둡게 두고 `.tgt`만 초록 — `patternPayoffHtml`의 `.pf-ex-en`은 '전부 초록'이 의도라 클래스를 분리했다). 근거를 코퍼스 수치가 아니라 **자기 산출물**로 두는 것이 요점이다.
