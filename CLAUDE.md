...
- `scripts/build_vocab_expression.py` — **어휘→표현** 프로토타입 데이터 생성(레포 밖 CSV/xlsx → `public/vocab-expression.data.json`). 소스 경로는 `VE_EXPR_CSV`/`VE_GRAM_CSV`/`VE_VOCAB_CSV`/`VE_GSE_XLSX` 환경변수(기본 `~/Downloads`). 조인: 표현/문법 CSV의 `content_word`(트리거 어휘)+`sentence`(배열 문장)를 **어휘 CSV(`All_*.csv`)**에 `dictSeq==seq`로 조인해 어휘 학습 데이터(`learnSentence` 영어 예문의 `[단어]`=빈칸 정답, `learnSentenceMeaning` 한국어 번역의 `[뜻]`=초록 하이라이트, `meaning`, `pos`)를 붙이고, gse xlsx에서 CEFR만 보강. 레벨(unit/level 1~30)별로 어휘가 겹치지 않게 `VE_ITEMS_PER_LEVEL`(기본 5)개씩 추출. 어휘 CSV의 `filter`가 `sexual`/`unnecessary`인 행은 제외. 표현 제목(`title_en`/`title_kr`, `~` 슬롯 패턴)·문법 소제목(`sub`)은 병합셀이라 **forward-fill**하고(문법은 큰제목 바뀌면 소제목 리셋), 같은 패턴(제목/소제목)의 다른 트리거 단어를 아이템마다 `siblings`(최대 3개)로 모아 붙인다.
- `public/vocab-expression.html` — **별도** 어휘→표현 프로토타입(자체 완결형 HTML, 본 앱과 무관, `/vocab-expression.html`로 서빙). 앱(말해보카)의 어휘/문법 학습 UX를 따른다. 흐름은 아이템마다 **[1] 어휘 빈칸 채우기 → [2] 문장 배열**. 홈은 **표현/문법 탭** × **레벨 카드**(완료는 `localStorage` `ve:cleared`로 ✓ 배지·`n/N 완료`, 마지막 탭은 `ve:mode`). [1] 어휘 카드는 채팅 말풍선에 **한국어 번역(정답 뜻 영역 초록 `.tgt` 하이라이트 + 괄호 설명줄 muted)** 위, 영어 예문의 `[단어]`를 빈칸으로 아래에 보여준다. 힌트는 **순차 노출** — 처음엔 `💡 힌트 보기`만, 누르면 첫 글자+길이가 드러나며 그때 `👁 정답 보기`가 등장(전체 노출해도 직접 입력해야 함). **정답을 입력하면 확인 없이 즉시 정답 처리**(`norm`: 대소문자·앞뒤 문장부호 무시), 오답은 `정답과 N글자 달라요`로 틀린 글자를 빨강 강조(`diffSpell`). 빈칸 정답은 학습 대상 표제어와 같아야 하므로 굴절형·부분구 예문(be→[am] 등)은 빌드에서 제외. [2]는 방금 배운 어휘가 트리거하는 `sentence`(`[...]{...}` 마크업 제거)를 단어 타일로 섞어 배열(타일 소문자 — 첫 단어 노출 방지, 종결부호 제거; **방해 단어는 현재 없음** — 데이터 준비되면 추가 예정). **패턴은 배열 전엔 숨긴다**(미리 보여주면 정답 구조가 새어나감) — 배열 전 트리거 박스(`triggerBoxHtml`)는 "방금 배운 {word}가 들어간 실제 문장이에요"처럼 단어 연결만 하고, **정답을 맞힌 뒤 보상으로 패턴을 공개**한다(`patternPayoffHtml`): "이건 {패턴} 패턴이에요"(표현: `title_en`(`~`)·`title_kr` / 문법: 소제목 문법 포인트) + 같은 패턴의 다른 단어(`siblings`) "…에도 그대로 써요". 단어가 부수적 필러인 경우(예: summer)의 어색함을 없애고, 재사용 가능한 패턴을 배운다는 효용감을 준다. 정답 시 원문 대소문자로 복원+트리거 초록 강조. 데이터 스키마: `modes.{expression,grammar}.levels[].items[]` = `{word,meaning,pos,cefr,vocab{answer,enLines,koLines},sentence{en,kr,trigger,pattern_en?,pattern_kr,siblings,...}}`. TTS 없음(리딩·타이핑 중심).
